...
/** 反问卡片最上面那行状态：已选几题、一共几题。 */
export const QUESTION_PROGRESS_TEXT = (answered, total) => `已选 ${answered}/共 ${total} 题`;

/** 点了「确定」但还有题没选时，顶上那行换成这句。 */
export const QUESTION_INCOMPLETE_TEXT = (count) => `还有 ${count} 题没选`;

/** 答完提交之后那张卡的标题。 */
export const QUESTION_DONE_TITLE = '回答已提交';

/** 提交之后卡片正文里的一行：哪一题答了什么。 */
...
/** 设置页 JSON 路由：只允许本机访问时的报错。 */
export const HTTP_LOOPBACK_ONLY_ERROR = '只允许本机访问';
/** 设置页 JSON 路由：请求方法不对时的报错。 */
export const HTTP_METHOD_ONLY_ERROR = (method) => `只接受 ${method}`;
/** 设置页 JSON 路由：请求体过大时的报错。 */
export const HTTP_BODY_TOO_LARGE_ERROR = '请求体过大';
/** 设置页 JSON 路由：请求体不是合法 JSON 时的报错。 */
export const HTTP_BODY_NOT_JSON_ERROR = '请求体不是合法 JSON';
