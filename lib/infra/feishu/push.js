...
export function createPush({ logger, client }) {
   ...
   *
   * 业务失败（`code !== 0`，例如卡片表格数量超限）不重试，但和放弃一样都会把原因交给 `onFailure`——
   * 有些失败调用方要换一种方式回话，光写日志用户什么都看不到。
   *
   * @param request 拿到客户端后真正要发的请求
   * @param describe 日志里用来描述这次发送的短句
   * @param onFailure 失败原因回执（飞书返回的 `code` / `msg`，或抛错信息）
  ...
  async function call(request, describe, onFailure) {
    ...
    if (!live) {
      ...
      onFailure?.({ message: '凭据未配置，消息无法发到飞书' });
      return undefined;
    }
