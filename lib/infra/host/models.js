 ...
 */
export function createModelCatalog({ logger, access }) {
  /**
   * 读模型目录。
   *
  ...
  async function current(sessionId) {
    ...
    try {
      ...
      const state = access.service('sessionProjections')?.stateOf?.(resolved?.agent?.session, 'modelSelection');
   ...
   * @returns `{ ok: true, selected }`，或 `{ ok: false, unsupported: true }` /
   *   `{ ok: false, message }`
   */
  async function select(request) {
    const selectModel = access.method('sessionController', 'selectModel');
    if (selectModel === undefined) return { ok: false, unsupported: true };
    try {
