...
import { readSettings } from '../plugin/config.js';
 ...
 */
export function createWorkspaces({ ctx, logger }) {
  /**
   * 列已注册的工作区。
   *
  ...
  function list() {
    ...
    if (!registry || typeof registry.list !== 'function') {
      logger.warn('宿主没有工作区注册表，工作区列表读不出来');
      return [];
    }
    try {
  ...
  /**
   * 当前工作区：设置里选过的那个；没选过就用当前会话所在的工作区。
   *
   * @returns 工作区 ID；两边都认不出时为空串
   */
  function currentWorkspaceId() {
    const { workspaceId, sessionId } = readSettings();
    if (workspaceId && list().some((workspace) => workspace.workspaceId === workspaceId)) {
      return workspaceId;
    }
