/** 会话目录：列本机的会话（按最近活动倒序）、查一个会话的名字与工作目录、打开会话、新建会话。 */

import { SESSION_CONTROL_MISSING_TEXT } from '../../copy.js';
 ...
 * @returns 会话目录：listRecent / titleOf / cwdOf / open / create / prompt / cancel / removeQueued
 */
export function createSessionCatalog({ ctx, logger, access, workspaces }) {
  let cached;
  ...
  async function prompt({ sessionId, requestId, mode, content }) {
    ...
    if (call === undefined) return { ok: false, error: SESSION_CONTROL_MISSING_TEXT };
    try {
  ...
  return { listRecent, titleOf, cwdOf, open, create, prompt, cancel, removeQueued };
}
