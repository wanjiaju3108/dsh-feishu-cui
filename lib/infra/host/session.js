 ...
 * @returns 会话目录：listRecent / titleOf / cwdOf / open / create / rename / prompt / cancel / removeQueued / hasSession
 */
export function createSessionCatalog({ logger, access, workspaces }) {
   ...
   * @returns `{ ok: true }`，或 `{ ok: false, error, code }`（`code` 是宿主那个 `RemoteError`
   *   自带的码，认不出时 undefined）
   */
   ...
   * 这条会话在不在会话列表里。
   *
   * 宿主没挂 `sessionQuery` 时判不了，按「在」返回——那种宿主上会话是好的，判成不在等于把插件废掉。
   *
   * @param sessionId 会话 ID
   * @returns 在（或者判不了）时 true
   */
  async function hasSession(sessionId) {
    if (!sessionId) return false;
    if (!access.service('sessionQuery')) return true;
    return (await all()).some((session) => session.id === sessionId);
  }

  return {
    listRecent, titleOf, cwdOf, open, create, rename, prompt, cancel, removeQueued, hasSession,
  };
}
