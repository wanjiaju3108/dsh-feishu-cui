 ...
 */
export function createSessionsHandler({ logger, push, catalog }) {
  /**
  ...
  async function switchToNewSession() {
    const { sessionId: previous, workspaceId } = readSettings();
    const target = await catalog.create({ cwd: await catalog.cwdOf(previous), workspaceId });
