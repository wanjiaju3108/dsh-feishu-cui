...
import {
  ...
  ANSWER_TABLE_LIMIT_TEXT,
  ANSWER_TABLE_LIMIT_TITLE,
...
/** 飞书因为卡片里的表格数量超限而拒收时的错误码（外层 `230099`，内层 `11310`）。 */
const CARD_TABLE_LIMIT_CODES = new Set([230099, 11310]);

/**
 * 判断这次发送失败是不是"卡片里的表格数量超限"。
 *
 * 飞书把内层原因塞在外层的 `msg` 里（`Failed to create card content, ext=ErrCode: 11310; ErrMsg: card table
 * number over limit`），所以外层码、内层码和文案三种特征都要认。
 *
 * @param failure `push` 回执的失败原因
 * @returns 是表格超限时 true
 */
function isCardTableLimit(failure) {
  if (!failure) return false;
  if (CARD_TABLE_LIMIT_CODES.has(failure.code)) return true;
  return /11310|table number over limit/i.test(failure.message ?? '');
}
...
export function createAnswer({ logger, push, catalog }) {
   ...
   *
   * 两种发不出去的情况都会把卡片换成提示卡并记账，不再往下画：正文超过 `CARD_MAX_BYTES` 记 `tooLong`；
   * 飞书因为表格数量超限拒收（230099 / 11310）记 `tableLimit`——那时内容一个字都没上屏，只能让人去网页端看。
   *
   * @param requestId 飞书消息 ID
  ...
  async function paint(requestId, compute) {
    ...
    if (record.tooLong || record.tableLimit) return;
    ...
    if (!updated && isCardTableLimit(failure)) {
      logger.warn(
        `回答卡片被飞书拒收（${requestId}，表格数量超限）：code=${failure?.code} msg=${failure?.message}`,
      );
      writeAnswerCard(requestId, {
        ...record,
        tableLimit: true,
        title: ANSWER_TABLE_LIMIT_TITLE,
        content: ANSWER_TABLE_LIMIT_TEXT,
      });
      await push.patchCard(record.cardId, buildHeaderTextCard({
        title: ANSWER_TABLE_LIMIT_TITLE,
        color: colorOf(ANSWER_TABLE_LIMIT_TITLE),
        content: ANSWER_TABLE_LIMIT_TEXT,
      }));
      return;
    }
