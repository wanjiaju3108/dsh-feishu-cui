...
/** 会话重命名输入框卡片的标题。 */
export const SESSION_RENAME_CARD_TITLE = '会话重命名';

/** 会话重命名输入框卡片标题下面那段说明。 */
export const SESSION_RENAME_CARD_DESCRIPTION = '给当前会话换个名字，填在下面这个输入框里。';

/** 新名字输入框的占位文案。 */
export const SESSION_RENAME_CARD_PLACEHOLDER = '新名字';

/** 点到「确定」之后卡片上先写的话：请求交出去了，结果等宿主那条事件来说。 */
export const SESSION_RENAME_REQUESTED_TEXT = (title) => `已请求：会话改名为【${title}】。`;

/** 用户在重命名卡片上点了「取消」时回的话。 */
export const SESSION_RENAME_CANCELLED_TEXT = '会话重命名已取消';

/** 改名没成时回的话。 */
export const SESSION_RENAME_FAILED_TEXT = '会话改名失败';

/** 表单卡片通用的「确定」按钮。 */
export const CONFIRM_TEXT = '确定';
/** 表单卡片通用的「取消」按钮。 */
export const CANCEL_TEXT = '取消';
/** 输入框卡片通用的那句「这卡失效了」：配对、会话重命名这两张卡用。 */
export const INPUT_CARD_STALE_TEXT = '这张卡片已失效，请重新点菜单';
...
/** 消息没能交给会话、而且宿主明说这个会话已经不在了时回的话。 */
export const MESSAGE_SESSION_GONE_TEXT = '当前会话已经不在了，重新打开会话列表选一个';
...
/** 回答卡片：飞书因为卡片里的表格数量超限而拒收（外层 230099、内层 11310）时的标题。 */
export const ANSWER_TABLE_LIMIT_TITLE = '任务失败';

/** 飞书因为表格数量拒收时卡片上写的这句，让人去网页端看，或者重问一次。 */
export const ANSWER_TABLE_LIMIT_TEXT = '回答里的表格太多，飞书拒收这张卡片；请到网页端查看这一轮，或者重新问一次';
...
/** 会话设置被改了：会话名已修改的标题。 */
export const SETTINGS_CHANGED_SESSION_NAME_TITLE = '会话名已修改';
...
/** 会话设置被改了：会话名改成了什么。 */
export const SETTINGS_CHANGED_SESSION_NAME = (title) => `会话名改成【${title}】`;
