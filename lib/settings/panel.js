...
import { CREDENTIALS_ROUTE, SLEEP_GUARD_ROUTE, STATE_ROUTE, UNBIND_USER_ROUTE } from './routes.js';
 ...
 * @param deps.readSleepGuard 读防休眠现场：`() => ({ supported, enabled, holding, pid, error })`
 * @param deps.writeSleepGuardEnabled 把开关的新值写进配置：`(enabled) => Promise<void>`
 * @param deps.startSleepGuard 起防休眠：`() => void`
 * @param deps.stopSleepGuard 停防休眠：`() => void`
...
export function createSettingsPanel({
  ...
  function register() {
    ...
    registerJsonRoute({
      ctx,
      webServer,
      path: SLEEP_GUARD_ROUTE,
      method: 'POST',
      handle: async (body) => {
        const enabled = body?.enabled === true;
        await writeSleepGuardEnabled(enabled);
        if (enabled) startSleepGuard();
        else stopSleepGuard();
