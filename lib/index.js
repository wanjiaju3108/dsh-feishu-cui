...
import { createSleepGuard } from './infra/system/sleep-guard.js';
import { createWorkspaces } from './infra/host/workspace.js';
import { initCurrentSession } from './init.js';
import { readConfiguredSessionId, readSettings, registerSettings, writeSettings } from './infra/plugin/config.js';
...
/** 插件条目的 config：当前会话、绑定的 user、当前工作区、防休眠开关。 */
...
export async function apply(ctx, config) {
  ...
  // 初始化
  await initCurrentSession({ logger, catalog, readConfiguredSessionId });
  ...
  const sessions = createSessionsHandler({ logger, push, catalog, workspaces });
  ...
  const sleepGuard = createSleepGuard({
    logger,
    isEnabled: () => readSettings().sleepGuardEnabled,
  });
  ...
  const panel = createSettingsPanel({
    ...
    readSleepGuard: sleepGuard.snapshot,
    writeSleepGuardEnabled: (enabled) => writeSettings({ sleepGuardEnabled: enabled }),
    startSleepGuard: sleepGuard.start,
