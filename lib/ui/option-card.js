 ...
 * @returns 元素列表
 */
function optionElements({ tag, requestId, groups, picked }) {
  const elements = [];
...
export function buildOptionCard({
  tag, requestId, title, groups, picked, confirmText, cancelText,
}) {
  const pickedValue = groups.length > 0 ? picked.get(groups[0].id) ?? '' : '';
  return optionCard({
    title,
    elements: optionElements({ tag, requestId, groups, picked }),
    buttons: [
...
export function buildOptionCardWithAdd({
  tag, requestId, title, groups, picked, confirmText, addText, cancelText,
}) {
  const pickedValue = groups.length > 0 ? picked.get(groups[0].id) ?? '' : '';
  return optionCard({
    title,
