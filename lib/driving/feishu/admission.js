...
import {
  NOT_MATCHED_REASON,
  NO_SESSION_REASON,
  SESSION_SWITCH_BUSY_REASON,
  UNSUPPORTED_MESSAGE_REASON,
  WORKSPACE_SWITCH_BUSY_REASON,
} from '../../handler/feishu/warn.js';
...
export function createAdmission({ readSettings }) {
   ...
   * @returns 没通过的原因；不该挡时为空串
   */
  function switchBlockedReason(tag) {
    if (!hasAnswerCard()) return '';
    if (tag === SESSION_LIST_KEY) return SESSION_SWITCH_BUSY_REASON;
    if (tag === WORKSPACES_KEY) return WORKSPACE_SWITCH_BUSY_REASON;
    return '';
  }
  ...
  function admitMessage(cuiEvent) {
    ...
    if (!isUser(operatorId)) {
      return { code: 2, reason: NOT_MATCHED_REASON };
    }
    if (!content) {
      return { code: 2, reason: UNSUPPORTED_MESSAGE_REASON };
    }
    if (!readSettings().sessionId) {
      return { code: 2, reason: NO_SESSION_REASON };
    }
    return { code: 0 };
  }
  ...
  function admitMenu(cuiEvent) {
    ...
    const blocked = switchBlockedReason(cuiEvent.tag);
  ...
  function admitCard(cuiEvent) {
    const { userId } = readSettings();
    if (!userId) {
      return cuiEvent.tag === PAIRING_KEY ? { code: 0 } : { code: 2, reason: NOT_MATCHED_REASON };
    }
